function updateStatus() {
  if (navigator.onLine) {
    ...
    $("body")
      .before()
      .html(
        '<div class="rounded bg-dark mx-3 mx-md-4 mx-sm-4 p-2 text-center text-success">Back Online</div>'
      );
    ...
    $("body")
      .before()
      .html(
        '<div class="rounded bg-dark mx-3 mx-md-4 mx-sm-4 p-2 text-center text-danger"><i class="fas fa-wifi"></i><br>Your are currently offline</div>'
      );
    $(".internet-status-container").css("display", "block");
  }
}
...
var divPop = $(
  '<button class=" btn rounded-lg " id="random" onclick="playPopSound()"></button>'
);
...
$("#prev-scores").html(
  `<div class='col h4'>${prevScores
    .map(function (val) {
      return `<span class='text-success badge badge-dark p-2'>${val}</span>`;
    })
    .join(" ")}</div>`
);
...
function randomDivShow() {
  divPop.css({
    top:
      Math.random() * ($("#play-area").height() - divPop.height() - 30) + "px",
    left:
      Math.random() * ($("#play-area").width() - divPop.width() - 30) + "px",
  });
  changeBg();
...
$("#start").on("click", () => {
  ...
  if (duration == "unset" || roundTime == "unset") {
    ...
    $("#play-area").html(
      `<span class="info-msg h3 text-white" id='dots'>Starting</span>`
    );
...
function stopGame() {
  ...
  $("#play-area").html(
    '<span class="info-msg h3 text-danger"><img src="./img/game-over.png" style="height:90px; width:90px;" /><br>Game Over</span>'
  );
  $("#curr-score").html(`0`);
  ...
  setTimeout(() => {
    ...
    if (localStorage.getItem("lastScore") > 100) {
      $("#score-modal-body #performance-msg").html(`
